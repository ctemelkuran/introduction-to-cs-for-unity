    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            while (input[0] != 'q')
            {
                ...
                float hypo = (float)Math.Sqrt(Math.Pow(deltaX, 2) + Math.Pow(deltaY, 2));

                float radianAng = (float)Math.Atan2(deltaY, deltaX);
                float degreeAng = radianAng * 180 / (float)Math.PI - 180;

                if(degreeAng <= -180)
                {
                    degreeAng += 360;
                }
                Console.WriteLine(hypo + " " + degreeAng);
